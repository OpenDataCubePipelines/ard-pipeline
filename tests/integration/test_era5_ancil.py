...
def output_filename(canberra_scene_sentinel2_path):
    ...
    out = f"{bn}.testing.wagl.h5"
    return out


def test_collect_era5_ancillary(
    canberra_scene_sentinel2_container, nci_era5_dir_path, output_filename
):
    ...
    cfg_paths = {
        ...
        "brdf_dict": {"brdf_path": "/g/data/v10/eoancillarydata-2/BRDF/MCD43A1.061"},
    }
    ...
    with h5py.File(dest, "w") as fid:
        ...
        ancillary.collect_era5_ancillary(
            canberra_scene_sentinel2_container, nci_era5_dir_path, cfg_paths, out_group
        )
