    ...
    for lat, lon in tiles:
        ...
        try:
            ...
            if cache_path.exists():
                buffer = MemoryFile(
                    cache_path.read_bytes(), filename=os.path.basename(key)
                )
...
def write_mosaic_tiff(
    ...
    if len(mosaic_data.shape) == 2:
        mosaic_data = mosaic_data.reshape(
            (1, mosaic_data.shape[0], mosaic_data.shape[1])
        )
...
def get_dem_for_acquisition_crs_buffer(dataset: Acquisition, buffer_distance=15000):
    """
    ...
    mosaic_pixelsize = (
        mosaic_px11_point[0] - mosaic_px00_point[0],
        mosaic_px00_point[1] - mosaic_px11_point[1],
    )
    ...
    if use_gdal_transform:
        dem_transform, dem_width, dem_height = calculate_default_transform(
            ...
            dem_corner_latlon[0],
            dem_origin_latlon[1],
            dst_width=dem_shape[-1],
            dst_height=dem_shape[-2],
        )
        ...
        dem_data, dem_transform = reproject(
            mosaic_data,
            destination=np.empty(dem_shape, dtype=mosaic_data.dtype),
        ...
        dem_pixelsize = (
            dem_px11_point[0] - dem_px00_point[0],
            dem_px00_point[1] - dem_px11_point[1],
        )
    ...
    write_mosaic_tiff(
        str(
            test_cache
            / (
                "test_reproj2_gdal_border.tif"
                if use_gdal_transform
                else "test_reproj2_raa_border.tif"
            )
        ),
        dem_data.reshape((1, dem_data.shape[0], dem_data.shape[1])),
        new_dem_geobox.transform,
        new_dem_geobox.crs,
        mosaic_nodata,
    )
...
def get_dem_for_acquisition(dataset: Acquisition, border_degrees: float):
    """
    Get Copernicus DEM for a specified data acquisition.
    ...
    ds_border = [
        *_crs_transform(border_ll, COP30M_CRS, ds_geobox.crs),
        *_crs_transform(border_ur, COP30M_CRS, ds_geobox.crs),
    ]
    ...
    ds_border_size_px = (
        int(ds_border_size[0] / dataset.resolution[0]),
        int(ds_border_size[1] / dataset.resolution[1]),
        int(ds_border_size[2] / dataset.resolution[0]),
        int(ds_border_size[3] / dataset.resolution[1]),
    )
    ...
    if use_gdal_transform:
        new_dem_transform, new_dem_width, new_dem_height = calculate_default_transform(
            ...
            border_ur[0],
            border_ur[1],
            dst_width=new_dem_shape[-1],
            dst_height=new_dem_shape[-2],
        )
        ...
        new_dem_data, new_dem_transform = reproject(
            dem_data,
            destination=np.empty(new_dem_shape, dtype=dem_data.dtype),
        ...
        new_dem_pixelsize = (
            new_dem_px11_point[0] - new_dem_px00_point[0],
            new_dem_px00_point[1] - new_dem_px11_point[1],
        )
        new_dem_geobox = GriddedGeoBox(
        ...
        new_dem_geobox = GriddedGeoBox(
            shape=new_dem_shape,
            origin=ds_geobox.convert_coordinates(
                (-ds_border_size_px[0], -ds_border_size_px[1])
            ),
    ...
    write_mosaic_tiff(
        str(
            test_cache
            / (
                "test_reproj_gdal_border.tif"
                if use_gdal_transform
                else "test_reproj_raa_border.tif"
            )
        ),
...
def get_dem_for_acquisition_wagl_slow(dataset: Acquisition, border_degrees: float):
    """
    ...
    reproj_dem_origin = ds_geobox.convert_coordinates(
        (-ds_border_size_px[0], -ds_border_size_px[3])
    )
    ...
    write_mosaic_tiff(
        ...
        reproj_dem_geobox.transform,
        reproj_dem_geobox.crs,
        dem_nodata,
    )
...
def get_dsm_hdf_for_aquisition(
    ...
    if use_wagl:
        dem_data, dem_geobox = get_dem_for_acquisition_wagl_slow(
            dataset, border_degrees
        )
    else:
    ...
    kwargs = compression.settings(
        filter_opts,
        chunks=(
            (1, dem_data.shape[1]) if dataset.tile_size[0] == 1 else dataset.tile_size
        ),
    )
    ...
    desc = "A subset of a DSM smoothed with a gaussian " "kernel."
